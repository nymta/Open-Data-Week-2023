...
with col2:
    ...
    with st.expander("Visualization fields"):
...
tab1, tab2, tab3 = st.tabs(["Data", "Line Chart", "Bar Chart"])
...
with tab3:
    ...
    # lets customize the color schemes - https://altair-viz.github.io/user_guide/customization.html#customizing-colors
    range = ['blue', 'green', '#FD71FF', '#A2A2A2', 'red']
    domain = ['Bronx', 'Brooklyn', 'Queens', 'Staten Island', 'Manhattan']
    ...
    bus_bar = (
        basechart.mark_bar().encode(
            ...
            y = alt.Y(field = select_metric, type='quantitative', aggregate=aggregate),
            # don't need the color parameter - but because we want a custom color scheme, we'll add it in
            color = alt.Color(
                field = 'borough', 
                scale = alt.Scale(
                    domain = domain,
